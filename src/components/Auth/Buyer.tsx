...
const AuthPage = () => {
  ...
  const [Family_size, setFamily_size] = useState(0);

  async function handleSignup(event: any) {
    event.preventDefault();
    try {
      const res = await axios.post('/api/signup', {
        Name, Password, Phone_no, ID_proof, Marital_status, Family_size
      });
      ...
      <div className="bg-white p-10 rounded-2xl w-96">
        ...
        {isSignIn ? (
          ...
          <form onSubmit={handleSignup}>
            ...
            <div className="mb-4">
              <label className="block text-gray-800 mb-1" htmlFor="maritalStatus">
                Marital Status
              </label>
              <select
                id="maritalStatus"
                ...
                className="bg-white text-gray-800 w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
              >
            ...
            <div className="mb-4">
              <label className="block text-gray-800 mb-1" htmlFor="familySize">
                Family Size
              </label>
              <input
                type="number"
                id="familySize"
                value={Family_size}
                onChange={(e) => setFamily_size(Number(e.target.value))}
                className="text-gray-800 w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
